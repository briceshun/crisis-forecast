...
# %% Date Range
startdate = datetime.strptime('2021-12-01', '%Y-%m-%d')
...
for i, j in zip (dates, dates1):
    ...
    try:
        vid, id = youtubeSearch(query='ukraine', key=key, start=i, end=j)
        dd = i
    except noVideos:
        print("No Videos")
...
with open("data/ukraine_videos.json", "w") as outfile:
...
# %%
with open("data/ukraine_stats.json", "w") as outfile:
...
for i, j in zip(range(0, len(idx), 100), list(range(99, len(idx), 100))+[(len(idx))]):
    ...
    with open(f"data/comments/ukraine_comments_{j}.json", "w") as outfile:
...
counter = 1
for i in [x for x in os.listdir('data/comments') if x[:7]=='ukraine']:
    # Read file
    print(f"{counter} - {i}")
    ...
    # Save
    with open(f"data/comments/processed/ukraine_comments_{counter}.json", "w") as outfile:
        json.dump(x, outfile)
